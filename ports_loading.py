"""Einlesen und Bereinigen von ports.csv (LINERLIB, github.com/blof/LINERLIB).
...
Etliche Zeilen haben Longitude/Latitude vertauscht, und zwar nicht nach
einem festen Muster (manche Zeilen sind korrekt, manche nicht) - eine
Distanz- oder Wertebereichs-Heuristik allein greift zu kurz, siehe
MEMO.md ("Argentia"/"Belem"-Fälle). Deshalb wird stattdessen die im
Datensatz angegebene "Country"-Spalte als Kontrolle genutzt: für beide
möglichen Lesarten (so wie im File, und vertauscht) wird per Offline-
Reverse-Geocoding das Land der Koordinate bestimmt; welche Lesart zum
